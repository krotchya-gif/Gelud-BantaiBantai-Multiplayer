...
- Render buffer dihitung dari ukuran canvas dalam CSS pixel dan memakai skala render: Low 0,90; Medium 1; High 1,25; Ultra 2 pixel render per CSS pixel. Skala tidak berubah mengikuti `devicePixelRatio`; anggaran pixel membatasi buffer akhir bila perlu. Tidak ada floor native HD/HD+; viewport CSS 360 × 825 menghasilkan sekitar 324 × 743 saat Low dan 720 × 1.650 saat Ultra selama batas pixel mengizinkan. Anggaran coarse-pointer 1,5 juta pixel dan perangkat low-end 1 juta; desktop/fine-pointer 5 juta. FPS singkat di bawah 60 tidak mengubah resolusi. Setelah dua jendela berturut-turut di bawah 60, kurangi partikel; shadow WebGL ikut dimatikan dan pulih setelah dua jendela di atas 68. Low mengurangi geometri rumput, firefly, debris, bloom, dan shadow, serta tidak diturunkan otomatis. High baru berpindah ke Medium setelah dua jendela berturut-turut rata-rata 30 FPS atau kurang. Pergantian otomatis tidak memunculkan toast. WebGPU tidak membangun ulang composer WebGL dan topologi empat point light/shadow tetap stabil saat preset berubah.
- Map bernama wajib memakai blueprint map pack yang sama untuk renderer dan collision. Bila blueprint terpilih hilang/tidak valid, hentikan pergantian map dengan error yang jelas; jangan diam-diam membuat layout acak atau collision perkiraan. Bangun world baru sebelum melepas world lama agar kegagalan tidak mengosongkan arena.
...
Pada project ini integration test sebelumnya pernah gagal di sandbox karena `listen EPERM`, lalu lulus setelah bind localhost diizinkan. Jangan anggap kegagalan sebelum assertion sebagai bug simulasi; catat kondisi sandbox dan hasil sesudah bind bila test lobby dijalankan. Verifikasi renderer/map dan menu terbaru pada 26 September 2026 lulus: `npm run build`, `npm run test:characters`, `npm test -- --run` (26 file/207 test), dan `git diff --check`. Verifier mencakup 10 rig gameplay dan dua design rig, skill solo, lifecycle visual, auto-aim, roster bot, Super saat respawn, kebijakan kualitas, satu runtime Three.js, tekstur air, dan 24 map pada tiga seed. Build berisi 28 file/2.666.292 byte; `index.html` 20,32 kB (5,51 kB gzip), CSS aplikasi 63,10 kB (12,48 kB gzip), JavaScript utama 40,28 kB (14,29 kB gzip), runtime Three.js bersama 637,30 kB (165,22 kB gzip), renderer WebGPU 577,87 kB (160,87 kB gzip), dan Character Studio 7,49 kB (3,29 kB gzip). Vite masih memperingatkan chunk runtime/WebGPU di atas 500 kB. Smoke browser mengonfirmasi logo/menu di desktop dan dua viewport landscape, pemilihan River Fort, pergantian mode/brawler, serta Escape yang mengembalikan fokus; ini bukan pengukuran perangkat fisik. Poco F6/Redmi 12C, touch fisik, dua perangkat, soak, dan balance playtest masih perlu diuji.
