...
- WebGPU menjadi renderer utama di desktop dan mobile. WebGL2 tetap menjadi fallback otomatis jika WebGPU tidak tersedia atau gagal diinisialisasi. Render buffer mengikuti ukuran canvas CSS dengan skala Low 0,90, Medium 1, High 1,25, dan Ultra 2 render pixel per CSS pixel. Skala tidak bergantung pada `devicePixelRatio`; anggaran pixel tetap membatasi hasil pada 1,5 juta pixel untuk coarse-pointer, 1 juta pada perangkat low-end, dan 5 juta untuk fine-pointer. Tidak ada floor native HD/HD+.
- Setelah dua jendela FPS di bawah 60, partikel opsional dikurangi dan shadow WebGL dimatikan sementara; pulih setelah dua jendela di atas 68. High berpindah ke Medium setelah dua jendela berturut-turut rata-rata 30 FPS atau kurang. Low tidak diturunkan lagi dan resolusinya tidak berubah otomatis.
...
Contoh ukuran untuk viewport CSS 360 × 825. Hasil ini sama pada browser DPR 1 maupun 3 selama anggaran pixel tidak membatasi preset:

| Preset | DPR render relatif ke CSS | Buffer perkiraan | Partikel/debris | Firefly | Detail rumput (radial × tinggi) |
|---|---:|---:|---:|---:|---:|
| Low | 0,90× | 324 × 743 (241 ribu pixel) | 40% | 18 | 3 × 1 |
| Medium | 1× | 360 × 825 (297 ribu pixel) | 65% | 36 | 4 × 2 |
| High | 1,25× | 450 × 1.031 (464 ribu pixel) | 85% | 60 | 5 × 3 |
| Ultra | 2× | 720 × 1.650 (1,188 juta pixel) | 100% | 90 | 6 × 4 |

Persentase partikel berlaku pada batas efek yang diterima; saat preset berubah, partikel aktif di atas batas baru langsung dihentikan. Pada perangkat low-end, anggaran 1 juta pixel membatasi Ultra menjadi sekitar 661 × 1.514. WebGPU tetap memakai empat point light dan tidak menggunakan AO, bloom, MSAA, atau dynamic shadow. Perubahan preset WebGPU hanya memperbarui skala buffer serta LOD/efek, tanpa membangun ulang composer WebGL atau mengubah topologi lampu/shadow. Ukuran buffer dihitung dari canvas CSS aktual; ukuran panel fisik tidak disimpulkan dari viewport browser.
...
Verifikasi renderer/map pada 26 September 2026 lulus: `npm test -- --run` (26 file, 207 test), `npm run test:characters`, `npm run build`, dan `git diff --check`. Regression coverage memastikan satu runtime Three.js untuk WebGPU dan engine klasik; skala Low/Medium/High/Ultra 0,90/1/1,25/2 terhadap CSS; tidak ada composer rebuild saat quality WebGPU berubah; geometri rumput dipakai ulang; 24 arena cocok dengan collision pada tiga seed; River Fort/bridge valid; dan blueprint map bernama yang hilang tidak jatuh ke layout acak. Verifier juga menangkap konstanta `RepeatWrapping` pada kedua arah tekstur air. Menu Settings kini memiliki tab Gameplay, Graphics, dan Controls; browser QA memeriksa pergantian tab serta Escape, termasuk panel yang tetap berada di dalam viewport landscape 844 × 390 dan 667 × 375.

Build terbaru menghasilkan 28 file di `dist/` dengan total 2.666.292 byte. `index.html` berukuran 20,32 kB (5,51 kB gzip), CSS aplikasi 63,10 kB (12,48 kB gzip), JavaScript utama 40,28 kB (14,29 kB gzip), runtime Three.js bersama 637,30 kB (165,22 kB gzip), renderer WebGPU 577,87 kB (160,87 kB gzip), dan Character Studio 7,49 kB (3,29 kB gzip). Vite tetap memberi peringatan karena chunk runtime dan WebGPU lebih besar dari 500 kB.

Smoke browser WebGPU berhasil memilih dan memulai River Fort; permukaan air/objek procedural tampil, dan pergantian Low → Medium → High → Ultra tidak memunculkan layar recovery atau toast. Pada viewport CSS browser 1920 × 1080, buffer yang terlihat adalah 1728 × 972 (Low), 1920 × 1080 (Medium), 2400 × 1350 (High), dan 2981 × 1677 (Ultra efektif sekitar 1,55× karena batas 5 juta pixel fine-pointer). Ini memverifikasi jalur browser, bukan FPS atau performa perangkat fisik.

Menu utama menampilkan logo dua baris di tengah, tombol Play/Multiplayer, dan ringkasan brawler, mode, serta arena; katalog pilihan dibuka lewat dialog. Smoke UI pada 1280 × 800 desktop serta 844 × 390 dan 667 × 375 landscape menunjukkan logo dan seluruh pilihan utama tetap muat. Pemilihan mode, arena, dan brawler memperbarui ringkasan; Escape menutup dialog dan mengembalikan fokus. Pengukuran Poco F6/Redmi 12C, uji touch fisik, multiplayer dua perangkat, soak test, dan balance playtest belum dilakukan.

Pembaruan skill roster, 25 September 2026: seluruh sepuluh karakter memiliki dua skill aktif dengan state dan timer authoritative yang dicerminkan pada solo serta multiplayer. Blue Gojo menarik selama 2,4 detik dan orb/marker memudar pada batas timer yang sama. Infinity Barrier kembali setelah 5 detik dan hanya menahan satu hit instance. Kedua Domain aktif selama 4 detik. Sukuna kini berperan sebagai mage api dengan Dismantle range 10 dan Fuga range 9/13,5. Skill baru mencakup dash/defense, parry, trap, stealth, tembakan penembus cover, chain lightning, smoke concealment, dan recast Kunai. Super Gojo mendapat pose cast dan burst partikel; Malevolent Shrine memasang visual kuil procedural di kedua mode. Kuil disusun dari empat mesh statis gabungan dan tidak memakai dynamic shadow; efek cast Gojo menggunakan effect system yang sudah ada. Renderer menjaga jumlah dynamic light WebGPU tetap stabil. Kebijakan floor HD 25 September telah diganti pada 26 September dengan skala render relatif ke CSS dan anggaran pixel perangkat; Low kini memangkas geometri rumput dan efek opsional. Hasil benchmark Poco F6 belum tersedia. Skill 1/2 tersedia di desktop dan touch dengan cooldown/charge serta arah drag touch.
...
Preset kualitas WebGPU tidak membangun ulang composer post-processing yang hanya dipakai WebGL, menambah/menghapus lampu, atau mengubah shadow caster. WebGPU memakai empat point light pool tetap dan tidak merender dynamic shadow. Render buffer memakai skala relatif ke ukuran canvas CSS dan anggaran pixel: Low 0,90; Medium 1; High 1,25; Ultra 2 render pixel per CSS pixel. Skala ini tidak dikalikan atau dibatasi oleh `devicePixelRatio`; anggaran pixel membatasi buffer akhir saat diperlukan. Tidak ada floor native HD/HD+. Coarse-pointer dibatasi 1,5 juta pixel dan low-end mobile 1 juta; fine-pointer 5 juta. Dua jendela FPS di bawah 60 mengurangi partikel; WebGL juga mematikan shadow sementara dan memulihkannya setelah dua jendela di atas 68. High baru turun ke Medium setelah dua jendela berturut-turut rata-rata 30 FPS atau kurang; Low tidak diturunkan. Perubahan performa otomatis tidak mengubah resolusi dan tidak memunculkan toast. Map bernama yang kehilangan map pack tidak boleh memakai generator acak atau collision pendekatan; perpindahan arena membuat world baru dulu dan mempertahankan world sebelumnya bila generasi gagal.
...
public/engine/
  three-legacy.js              legacy roster, map/quality settings, and PCSS compatibility
...
File engine tetap dimuat sebagai script klasik berurutan karena kelas gameplay memakai namespace global. Runtime Three.js r186 kini berasal dari satu modul bersama di `src/three-global-runtime.js`; alias Vite mengarahkan impor `three` dan addon ke `three/src/Three.js`, dan renderer WebGPU memakai source tree yang sama. `three-legacy.js` memeriksa runtime itu lalu menyediakan roster baseline, aturan map/mode/kualitas, dan patch PCSS lama. Urutan script engine tetap sama.
