...
export function createSimulationState({ mode = 'deathmatch', mapId = 'open', mapSeed = 0, matchSeed = 0, players = [], spawnPoints = null } = {}) {
  ...
  const rules = {
    ...
    classic: { timeLimit: 205, respawn: false, respawnDelay: 0, targetKills: 0, gas: true },
    blitz: { timeLimit: 112, respawn: false, respawnDelay: 0, targetKills: 0, gas: true },
  }[mode] || { timeLimit: 300, respawn: true, respawnDelay: 5, targetKills: 50, gas: false };
  const state = {
    tick: 0,
    match: {
      mode,
